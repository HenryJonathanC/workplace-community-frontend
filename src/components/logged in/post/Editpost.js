...
const Editpost = ({handleCancel1, post}) => {
    ...
    const [link, setLink] = useState(post.links)
    const [videoLink, setVideoLink] = useState(post.videoLink)
    const [publisher, setPublisher] = useState(post.publisher)
    ...
    //   console.log(title + body + hashtags + category + publisher + link + videoLink)
      ...
      axios.post('http://localhost:3001/posts/edit', {
        "_id" : `${post._id}`,
        "userId" : `${userid}`,
        "title" : `${title}`,
        "body" : `${body}`,
    ...
    return (
        ...
        {/* {console.log(post)} */}
          <div className='mb-5 flex justify-between'>
            ...
            {image && <img className=' mt-3 w-40 h-40' src={image} alt="Uploaded.png" />}
